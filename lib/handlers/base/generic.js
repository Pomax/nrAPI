module.exports = function(options) {
  ...
  var processSQLwildcards = function(term) {
    return term.replace(/\*/g,'%').replace(/\?/g,'_');
  };
  ...
  return {
    ...
    findUsingKana: function(hiragana, katakana, next) {
      ...
      hiragana = processSQLwildcards(hiragana);
      katakana = processSQLwildcards(katakana);
      var findEntry = this.findEntry;
      var success = function(results) {
        var ids = results.map(function(v) { return v.id; });
        ids = ids.filter(function(elem, pos) {
